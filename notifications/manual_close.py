...
Was dieses Modul NICHT tut:
  ...
  * Es entscheidet nichts selbst. Aufgerufen wird es erst, nachdem der
    Nutzer zweimal unabhaengig bestaetigt hat.
  * Es kennt keine Bedienoberflaeche. Hier steht kein Telegram- und kein
    HTTP-Code; das Modul formatiert nichts und antwortet niemandem. Es
    gibt genau eine schreibende Funktion, und die ruft auf, wer die
    beiden Bestaetigungen eingesammelt hat - der Telegram-Bot
    (notifications/telegram_bot.py) oder das Dashboard
    (dashboard/schliessen.py). Beide teilen sich damit dieselbe
    Absicherung; eine zweite Umsetzung waere genau die Doppelfuehrung,
    die in diesem Projekt schon mehrfach dazu gefuehrt hat, dass zwei
...
# Womit der Eingriff ausgeloest wurde - steht als `quelle` im Protokoll.
...
# Wie lange eine angefangene Bestaetigung gilt. Ohne Frist koennte ein
# "Ja" von heute Morgen am Abend noch eine Position zu einem laengst
# veralteten Kurs schliessen. Bewusst HIER und nicht je Oberflaeche:
# Telegram und Dashboard sollen dieselbe Frist haben, sonst waere die
...
#
# EIN gemeinsames Protokoll fuer alle Oberflaechen, nicht je eines fuer
# Telegram und Dashboard. Die Frage, die im Zweifel beantwortet werden
# muss, lautet "wer hat diese Position wann von Hand geschlossen" - und
# die soll sich aus EINER Datei beantworten lassen, nicht aus zweien, die
# man vorher zusammensortieren muss. Woher der Eingriff kam, steht als
# Feld `quelle` in der Zeile.
...
def schliesse_position(bot_name: str, trade_id: int, exit_price: float,
                        ...
                        jetzt: str = None, quelle: str = None) -> dict:
    ...
    die Bestaetigung nicht umgehen koennen, nur weil er eine andere
    Oberflaeche baut.

    quelle: nur fuer das Protokoll - QUELLE_TELEGRAM oder
    QUELLE_DASHBOARD. Hat keinen Einfluss darauf, WAS geschrieben wird;
    die Datenbankzeile sieht in beiden Faellen gleich aus."""
    try:
        return _schliesse_position_ungeschuetzt(
            bot_name, trade_id, exit_price, benutzer_id, bestaetigungstext,
            jetzt, quelle)
...
def _schliesse_position_ungeschuetzt(bot_name, trade_id, exit_price,
    ...
    ergebnis = {
        ...
        "quelle": quelle or QUELLE_UNBEKANNT,
        "vorher": vorher,
        "nachher": nachher,
    }
