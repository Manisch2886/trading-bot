"""
Phase 4b - Taegliche E-Mail-Zusammenfassung: RSI-2 Mean-Reversion (Krypto)
================================================================================
Analog zu elliott_wave_stocks/daily_summary_email.py bzw.
t3_supertrend/daily_summary_email.py.
"""
...
def format_stop(stop_price) -> str:
    if stop_price is None or (isinstance(stop_price, float) and pd.isna(stop_price)):
        return "kein Stop"
    return f"{stop_price:.4f}"  # mehr Nachkommastellen als Aktien - Krypto-Preise reichen von <0.01 bis >100000
...
def build_summary_text(open_trades: pd.DataFrame, closed_trades: pd.DataFrame) -> str:
    ...
    if not new_positions.empty:
        for _, t in new_positions.iterrows():
            lines.append(f"  - {t['symbol']}: Entry {t['entry_price']:.4f}, "
                          f"Stop {format_stop(t['stop_price'])}, RSI(2) bei Einstieg "
                          f"{t['rsi_at_entry']:.1f}")
    lines.append("")
