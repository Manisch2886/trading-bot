"""
Phase 4b - Taegliche E-Mail-Zusammenfassung
================================================
Liest paper_trading.db aus (die von forward_test.py gepflegt wird)
und verschickt eine kurze, uebersichtliche Zusammenfassung per E-Mail:
- neu eroeffnete Positionen seit gestern
- geschlossene Trades seit gestern (mit Ergebnis)
- aktuell offene Positionen insgesamt
- Gesamt-Performance bisher (Win Rate, Summe PnL)

Gedacht zum taeglichen automatischen Aufruf via cron, direkt NACH
forward_test.py (siehe Cronjob-Anleitung).
"""
...
# email_config.py liegt im config/-Ordner, nicht neben diesem Skript -
# daher muss der Ordner explizit zum Suchpfad fuer Importe hinzugefuegt werden.
...
# Claude-Agenten-Zusatzfunktionen - kosten echtes Geld pro Aufruf (eigener
# ANTHROPIC_API_KEY noetig). Einzeln abschaltbar, falls nicht gewuenscht.
    ...
    if not new_positions.empty:
        for _, t in new_positions.iterrows():
            lines.append(f"  - {t['symbol']}: Entry {t['entry_price']:.2f}, "
                          f"Ziel {t['target_price']:.2f}, Stop {t['stop_price']:.2f}")
    lines.append("")

    # Geschlossene Trades seit gestern
...
if __name__ == "__main__":
    ...
    print(summary)  # auch lokal ausgeben, fuers Log
