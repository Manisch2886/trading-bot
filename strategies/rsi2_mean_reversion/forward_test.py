"""
Phase 4 - Forward Testing / Paper Trading: RSI-2 Mean-Reversion (Aktien)
================================================================================
...
protokolliert alles in einer EIGENEN Datenbank (automatisch benannt durch
strategy_paths.py). Zum WIEDERHOLTEN Ausfuehren gedacht (z.B. taeglich
nach Handelsschluss).

WICHTIG - anders als beim Elliott-Wave-Bot KEIN Freshness-Fenster fuer
Signale: RSI-2 scannt taeglich neu (kein mehrtaegiges "Wellenmuster", das
erst verzoegert erkannt wird), daher wird bei jedem Lauf einfach die
LETZTE verfuegbare Kerze auf ein Signal geprueft - identisch zum Muster
von t3_supertrend/forward_test.py. Entry-Preis = Schlusskurs der letzten
(heutigen) Kerze, exakt wie im validierten Backtest ("Einstieg zum
Schlusskurs des Signaltags").

DEFENSIVES None/UNBEGRENZT-HANDLING (Praezedenzfall: TypeError-Bug beim
Elliott-Wave-Aktien-Bot, als STOP_LOSS_PCT/MAX_CONCURRENT_POSITIONS dort
testweise auf None/unbegrenzt gesetzt wurden): STOP_LOSS_PCT=None ist hier
sogar der VALIDIERTE Live-Wert (kein Stop schneidet besser ab, siehe
PROTOTYPE_FINDINGS.md) - alle Vergleiche mit stop_price sind daher explizit
gegen None abgesichert, ebenso MAX_CONCURRENT_POSITIONS gegen None
("unbegrenzt").
"""
...
from live_params import RSI_THRESHOLD, STOP_LOSS_PCT, MAX_HOLD_DAYS, MAX_CONCURRENT_POSITIONS

INTERVAL = "1d"
LOOKBACK_PERIOD = "3y"  # genug Vorlauf fuer SMA(200) + Puffer
...
def check_open_trades(conn, indicator_data: dict):
    """Prueft offene Positionen Balken fuer Balken NACH dem Einstiegstag
    (Bar-Anzahl, keine Kalendertage - identisch zur Zaehllogik im
    validierten Backtest backtest_rsi2.run_backtest), Prioritaet pro Balken:
    ...
    for _, trade in open_trades.iterrows():
        ...
        stop_price = trade["stop_price"]  # kann None sein (SQLite NULL -> pandas NaN)
...
def find_new_signals(conn, indicator_data: dict):
    """Prueft die LETZTE verfuegbare Kerze jedes Symbols auf ein RSI-2-Signal
    (Trendfilter + RSI < Schwelle). Beruecksichtigt MAX_CONCURRENT_POSITIONS
    (None = unbegrenzt), konsistent mit equity_simulation.py.
    ...
    for symbol, df in indicator_data.items():
        ...
        try:
            ...
            pass  # Bereits erfasst (Cronjob evtl. zweimal am selben Tag gelaufen)
...
if __name__ == "__main__":
    ...
    print(f"RSI-Schwelle: {RSI_THRESHOLD}  |  Stop-Loss: "
