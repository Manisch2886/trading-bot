"""
Phase 2 - Multi-Symbol Walk-Forward-Validierung: Bollinger-Band-Squeeze-Breakout
======================================================================================
Analog zu rsi2_mean_reversion/multi_symbol_walk_forward.py: Split-Punkt wird
...
Rohhistorie - der beim RSI-2-Prototyp gefundene und behobene Fehler
(verzerrter Split bei alten Aktien wie NVDA) wird hier von Anfang an
vermieden.
"""
...
def split_all_symbols(all_data: dict, ratio: float) -> tuple:
    """Splittet den Split-Punkt innerhalb des Auswertungsfensters (entry_cutoff
    bis Datenende), nicht auf der vollen Rohhistorie - siehe Modul-Docstring."""
...
def evaluate_combination_multi_windowed(windowed_data: dict, stop_loss_pct: float,
...
def run_multi_optimisation_windowed(windowed_data: dict) -> pd.DataFrame:
    from multi_symbol_optimise import STOP_LOSS_RANGE
    results = []
