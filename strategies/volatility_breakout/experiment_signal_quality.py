"""
Volatility Breakout: Signal-Qualitaets-Test (analog zu RSI-2)
================================================================================
Trennt Timing-/Signal-Nutzen von Kapitalmanagement-Nutzen - identische
Methodik wie rsi2_mean_reversion/experiment_signal_quality_and_priority.py,
Aufgabe 1. Vergleicht:

(a) Ø PnL/Trade der TATSAECHLICH mit Kapital versorgten Trades (real,
    Limit 8, 10% Allokation - der urspruengliche validierte Startpunkt)
(b) Ø PnL/Trade ALLER gefundenen Signale (unabhaengig von Kapitalverfuegbarkeit)
(c) eine hypothetische Kapital-KEIN-Limit-Simulation (jedes Signal bekommt
    sofort 10% des aktuellen Kapitals, keine Pruefung auf freies
    Kapital/Positionslimit - REIN ZUR DIAGNOSE, unrealistisches Leverage,
    KEIN Vorschlag fuer echten Live-Betrieb).

Ziel: pruefen, ob (wie bei RSI-2 gefunden) die tatsaechlich ausgefuehrten
Trades im Schnitt sogar schlechter sind als die Gesamtpopulation - das
wuerde denselben Mechanismus bestaetigen (Kapitalerschoepfung genau in
Phasen mit vielen gleichzeitigen Signalen, keine neutrale Auswahl).
...
def signal_quality_test(all_trades: pd.DataFrame, label: str):
    ...
    print(f"\n(a) Tatsaechlich ausgefuehrte Trades (Limit {BASELINE_LIMIT}, "
          f"{BASELINE_ALLOCATION_PCT*100:.0f}% Allokation, Stop-Loss {STOP_LOSS_PCT}%):")
...
if __name__ == "__main__":
    print(f"Feste Parameter: Stop-Loss {STOP_LOSS_PCT}% (validierte Basiskonfiguration)\n")
