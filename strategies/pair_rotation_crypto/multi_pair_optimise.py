"""
Phase 1 - Multi-Pair-Optimierung: Relative-Strength-Pair-Rotation (Krypto)
================================================================================
Analog zu pair_rotation_stocks/multi_pair_optimise.py. Nutzt dasselbe
25-Coin-Binance-Universum wie die anderen Krypto-Bots (Tageskerzen).

Kein RECENT_YEARS_ONLY-Konzept (siehe rsi2_crypto/multi_symbol_optimise.py) -
die verfuegbare Krypto-Historie ist bereits auf ~5 Jahre begrenzt.
Stattdessen: Referenzdatum (Beginn des Auswertungsfensters, ab dem Trades
gezaehlt werden UND vor dem die Paar-Korrelation berechnet wird) =
frueheste gemeinsame Datenverfuegbarkeit + 252 Handelstage Korrelations-
Vorlauf.
"""
...
LOOKBACK_DAYS_RANGE = [20, 60, 120]
REBALANCE_DAYS_RANGE = [1, 5]
STOP_LOSS_RANGE = [None, 8.0, 12.0]

MIN_TRADES = 40
MIN_AVG_RETURN_PCT = 0.0
MIN_PAIRS_CONTRIBUTING = 4
MIN_HISTORY_DAYS = 1500  # bewusst hoeher als bei RSI-2/Volatility-Breakout-Krypto (500): dort ist
                          # der Vorlauf pro SYMBOL unabhaengig, hier zwingt ein gemeinsames
                          # Referenzdatum (siehe get_reference_date) den spaetesten Symbol-Start
                          # der GESAMTEN Universumsauswahl auf alle Paare durch - ohne diese hoehere
                          # Schwelle wuerden wenige sehr junge Coins (z.B. BMTUSDT, erst seit
                          # 2025-03) das Auswertungsfenster auf wenige Monate zusammenstauchen.
                          # 1500 Tage trennt sauber die 13 Coins mit voller ~5-Jahres-Historie
                          # (alle seit 2021-09-01) von juengeren Coins ab
...
def get_reference_date(symbol_data: dict) -> pd.Timestamp:
    """Frueheste gemeinsame Datenverfuegbarkeit + 252 Handelstage
    Korrelations-Vorlauf = Beginn des Auswertungsfensters (kein
    RECENT_YEARS_ONLY-Konzept bei Krypto, siehe Modul-Docstring)."""
    earliest_common_start = max(df["open_time"].min() for df in symbol_data.values())
    return earliest_common_start + pd.Timedelta(days=WARMUP_PERIOD)
...
if __name__ == "__main__":
    ...
    print(f"Auswertungsfenster beginnt: {reference_date.date()}\n")
