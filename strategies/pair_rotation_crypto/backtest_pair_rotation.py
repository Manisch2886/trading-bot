"""
Backtest-Engine: Relative-Strength-Pair-Rotation (Krypto)
================================================================================
Long-only, Krypto, Tagesbasis (siehe rsi2_crypto/backtest_rsi2.py-Docstring
fuer die Zeitrahmen-Begruendung - identisch anwendbar: Handelstage =
Kalendertage bei 24/7-Krypto). Rotiert innerhalb eines festen Krypto-Paares
...
Eigenstaendige Kopie von pair_rotation_stocks/backtest_pair_rotation.py
...
1. PAAR-AUSWAHL: siehe pair_discovery.py (Korrelation >= 0.70 ueber die
   252 Handelstage vor dem Auswertungsfenster - z.B. BTC/ETH als klassisches
   Ankerpaar, siehe Anfrage). Die Paar-Liste ist fuer die Dauer eines
   Backtest-Laufs FEST (kein laufendes Neu-Suchen von Paaren waehrend
   des Backtests).
...
if __name__ == "__main__":
    ...
    df_a = pd.read_csv(os.path.join(_P["DATA_DIR"], "BTCUSDT_1d.csv"), parse_dates=["open_time"])
    df_b = pd.read_csv(os.path.join(_P["DATA_DIR"], "ETHUSDT_1d.csv"), parse_dates=["open_time"])
    pair_df = compute_pair_indicators(df_a, df_b, lookback_days=60)
    trades = run_backtest(pair_df, "BTCUSDT", "ETHUSDT", lookback_days=60, rebalance_days=5, stop_loss_pct=8.0)

    print(f"BTCUSDT/ETHUSDT: {len(trades)} Trades gefunden.\n")
    if not trades.empty:
