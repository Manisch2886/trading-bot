...
*Nachtrag 2026-09-13 (Befund TB-17):* Die Abschnitte **3.3**, **4.2**, **8** und **9** (Punkte 4 und 18) sind auf diesen Tag nachgezogen — Anlass war eine veraltete Buy-and-Hold-Aussage in `strategies/elliott_wave_stocks/live_params.py` und die daraus folgende Entscheidung des Nutzers vom 12.09.2026. Der **übrige** Text beschreibt weiterhin den Stand nach PR #73 und hinkt den Merges #74–#88 hinterher.
...
18. **`results/elliott_wave_stocks/EXPERIMENT_FINDINGS.md` führt durchweg Look-Ahead-Zahlen** (Befund TB-17, 13.09.2026). Die Datei ist die Quelle des Take-Profit-Eintrags vom 2026-09-03 und nennt für die Live-Konfiguration +1500,5 % bei −1,90 %; nachgerechnet sind es **+352,72 % bei −22,44 %** (PR #86). Auch das Paar „mit/ohne Kursziel" (+1500,5 % gegen +3084,1 %) stammt vollständig von der look-ahead-behafteten Grundlage. Die **Entscheidung** `USE_TAKE_PROFIT = False` hält trotzdem — sie ruht seit PR #28 auf einer Rangfolge (unter den 116 Kombinationen, die die Mindestfilter bestehen, arbeiten die vorderen zehn praktisch ausnahmslos ohne festes Kursziel), nicht auf diesen Zahlen. Ein sauber gerechnetes Gegenstück zum Paar „mit/ohne Kursziel" existiert **nicht**; der gemessene Hebel der Entscheidung ist damit unbekannt. In `live_params.py` ist das jetzt vermerkt, in der Datei selbst **nicht** — sie liegt unter `results/` und beschreibt einen Lauf, der so stattgefunden hat. **Zu entscheiden:** Warnhinweis in die Datei, oder Verlass auf den Vermerk in `live_params.py`.
