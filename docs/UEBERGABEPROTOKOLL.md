...
*Nachtrag 2026-09-13 (TB-26, Zuteilungskaskade):* Die Frage **„wer bekommt den Platz, wenn er knapp wird?"** ist seit TB-26 beantwortet und steht an einer einzigen Stelle: `shared/zuteilung.py`. Vorher entschied bei bindendem Positionslimit oder knappem Kapital die **Zeilenreihenfolge des Trade-DataFrames** — also die Symbolreihenfolge von `config/sp500_top150.txt` bzw. `config/top25_symbols.txt` —, und ein Zweitkriterium gab es nicht. TB-23 hatte das gemessen: acht von neun Bots lieferten unter permutierter Symbolreihenfolge andere Ergebnisse, bei `volatility_breakout` schwankte die Rendite zwischen 145,16 % und 269,47 %. `shared/determinismus.py --voll --perms 20` meldet seither für **alle neun** Bots `DETERMINISTISCH`. Die Kennzahlen haben sich dadurch verschoben (die alte Zahl war eine von zwanzig, die neue ist eine bestimmte); Einzelheiten, Vorher/Nachher je Bot und die getroffenen Festlegungen stehen in **`docs/TB-26_ZUTEILUNGSKASKADE.md`** und `research/zuteilungskaskade/BERICHT.md`. **Die abgelegten `results/*/equity_curve.csv` sind bewusst nicht mit neu erzeugt** — `shared/ergebniskurven.py` meldet sie bis dahin zu Recht als ABWEICHEND.

*Nachtrag 2026-09-13 (TB-28, Messkette):* Die zweite Hälfte derselben Frage — **„wie wird aus einem Kapitalpfad eine Kennzahl?"** — steht seit TB-28 ebenfalls an einer einzigen Stelle: `shared/messkette.py`. `calculate_max_drawdown()` und die Renditeformel standen bis dahin **neunmal zeichengleich** in den `strategies/*/equity_simulation.py` (nachgewiesen in TB-27); die neun Dateien importieren sie jetzt, bleiben aber an ihren Pfaden und behalten ihre Signaturen — `elliott_wave` insbesondere weiterhin **ohne** `max_concurrent_positions`. Auch `shared/ergebniskurven.py::kennzahlen()` und `shared/determinismus_lauf.py` rechnen nicht mehr nach, sondern benutzen dieselbe Quelle. **Keine Zahl hat sich dadurch verschoben:** die neun frisch gerechneten Kurven sind vor und nach der Änderung SHA-256-gleich, und der `--json`-Bericht von `ergebniskurven.py` ist byte-identisch. Einzelheiten in **`docs/ERGEBNIS_TB-28_messkette.md`**, Testauftrag in `docs/TESTAUFTRAG_TB-28_messkette.md`. **Welches Mass führt, ist damit ausdrücklich NICHT entschieden** — das ist Schritt 3 der TB-27-Empfehlung und betrifft die *auswählende* Kennzahl in `multi_symbol_optimise.py` (9×), `multi_symbol_walk_forward.py` (6×) und den `optimise_*.py` (3×).

*Nachtrag 2026-09-14 (TB-30a, Vorregistrierung):* Die **Neuselektion der neun Bots ist vorregistriert**, aber **nicht gelaufen**. Das Register steht in **`docs/VORREGISTRIERUNG_neuselektion.md`**, der Nulltest S-E1 in `docs/VORREGISTRIERUNG_S-E1_nulltest.md`, das eingefrorene Auswertungsskript samt Vorab-Berechnungen unter `research/vorregistrierung/`. **Die Frage aus dem Nachtrag TB-28 — „welches Mass führt" — ist damit beantwortet:** es führt der **Kapital-Drawdown aus `equity_simulation.py`**, Selektionsstatistik ist der **Median des Netto-Sharpe über die Selektionsfalten** (Festlegungen des Betreibers vom 14.09.2026, vollständig in Abschnitt 1 des Registers). Zwei Folgen für den Alltag: **Agent 2** (`shared/param_search_agent.py`) wählt nicht mehr fest auf dem chronologischen Drawdown, sondern über eine dreistufige Kaskade, die heute noch beim Stellvertreter steht und von selbst auf das führende Mass umschaltet, sobald `multi_symbol_optimise` ihn mitliefert (TB-30b); und **`shared/regimewache.py`** hält die Entscheidung fest, dass ein fehlendes `BTCUSDT` den Lauf **abbricht** statt den Regimefilter stillschweigend zu überspringen — eingebaut wird sie in TB-30b, **vorher darf kein Selektionslauf starten**. Die fünf **Krypto-Faltenpläne sind Platzhalter mit Regel** und hängen an TB-31. Einzelheiten in `docs/UEBERGABE_TB-30a_vorregistrierung.md`, Testauftrag in `docs/TESTAUFTRAG_TB-30a_vorregistrierung.md`. **Es wurde kein Raster gerechnet und kein Parameter übernommen.**

*Nachtrag 2026-09-13 (Befund TB-17):* Die Abschnitte **3.3**, **4.2**, **8** und **9** (Punkte 4 und 18) sind auf diesen Tag nachgezogen, ebenso der Warnhinweis in `results/elliott_wave_stocks/EXPERIMENT_FINDINGS.md` — Anlass war eine veraltete Buy-and-Hold-Aussage in `strategies/elliott_wave_stocks/live_params.py` und die daraus folgende Entscheidung des Nutzers vom 12.09.2026. Der **übrige** Text beschreibt weiterhin den Stand nach PR #73 und hinkt den Merges #74–#88 hinterher.
...
**Seit TB-38 (15.09.2026) ist die Spalte „Datenquelle" die *zweite* Stufe, nicht die erste:** alle neun lesen zuerst `data/`, und nur wenn dort die erwartete Entscheidungskerze fehlt, rufen sie über die genannte Quelle live ab — und melden das. Ebenfalls seit TB-38 entscheiden alle neun auf der **letzten abgeschlossenen** Kerze statt auf der laufenden. Siehe Abschnitt 4.8.
...
**Entscheidung vom 12.09.2026 — der Bot läuft weiter, aber als Diversifikator.** Nicht weil er den Markt schlägt, sondern wegen seines Beitrags zum Portfolio; daran wird er künftig auch gemessen, nicht an der Einzelrendite. Prüftermine: **Quartals-Review Oktober 2026** als Zwischenstand, ein belastbares Urteil eher **Januar 2027** — die Live-Historie ist zu kurz (kein Bot erreicht `MIN_LIVE_CLOSED_TRADES = 10`, die Portfolio-Zahlen stammen also weiterhin aus Backtest-Kurven, siehe Abschnitt 9 Punkt 15). **Schärferes Prüfkriterium für diesen Termin:** der Bot bei **gleicher Zeit im Markt** gegen das **95. Perzentil von Zufalls-Timing**. Dieser Maßstab ist gewählt, aber **noch nicht gemessen** — er ist ein Auftrag an den Prüftermin, kein Ergebnis.

**Der Vorbehalt, der zu diesem Kriterium geführt hat:** `research/exposure_messung/BERICHT.md` misst für diesen Bot **95,4 % Zeit im Markt** und eine Korrelation von **+0,46** zu einem gleichgewichteten Buy-and-Hold des Aktienuniversums (tägliche Bewertung zu Marktpreisen). Ein Bot, der fast immer investiert ist und sich dabei deutlich mit dem Markt bewegt, ist kein selbstverständlicher Diversifikator — die Begründung „Diversifikator" darf beim Prüftermin also nicht ungeprüft durchlaufen. Dasselbe Dokument zeigt zudem, dass gerade die außergewöhnlich flache **abgelegte** Kurve dieses Bots (−1,65 %) den kombinierten Vierer-Drawdown getragen hat; frisch gerechnet sind es **−21,16 %**.
...
**Seit dem 13.09.2026 zusätzlich abgesichert: `shared/test_live_params_werte.py`** (Befund TB-17). Der Sync-Check vergleicht `live_params.py` mit dem Bot-Code — er beantwortet nicht, ob die Werte in `live_params.py` selbst noch dieselben sind. Diese Prüfung tut das: sie hält **alle 61 Konstanten aller neun Bots** als Sollwerte fest und meldet jede Abweichung namentlich. Dazu prüft sie, dass jede der neun Dateien **rein deklarativ** bleibt (nur Zuweisungen, jeder Wert ein Literal) — damit kann eine Änderung, die sich als „nur Kommentar" ausgibt, auch keine Logik einschleusen, die die Sollwert-Tabelle umgeht. Anlass war eine Dokumentations-Korrektur an `elliott_wave_stocks/live_params.py`: Wer die Datei anfasst, in der jeder Handelsparameter genau einmal steht, sollte nachweisen können, dass kein Wert dabei gewandert ist — ein Blick in den Diff zeigt, was jemand geändert hat, nicht, was herausgekommen ist.
...
1. Er lauscht auf **`100.106.38.8:8787`** (Tailscale-Adresse), **nicht** auf `localhost` — ein `curl http://localhost:8787/` läuft ins Leere, obwohl der Dienst einwandfrei läuft. Die Grundeinstellung im Code ist `127.0.0.1` (`dashboard/konfig.py`), die Adresse muss also von aussen gesetzt werden. **Seit TB-16 ist belegt, woher** (12.09.2026): `DASHBOARD_HOST` steht in der **`.env`**, `DASHBOARD_PORT` ist gar nicht gesetzt (es gilt `STANDARD_PORT = 8787`). Die laufende `plist` setzt keinen von beiden über `EnvironmentVariables` — die entsprechende Erwartung in `system/test_dienst_plists.py` war eine als „erschlossen" gekennzeichnete Vermutung und ist damit widerlegt; die Prüfung ist jetzt **bedingt** (setzt die Vorlage einen der Werte, muss er stimmen; setzt sie ihn nicht, ist das in Ordnung). **Daraus folgt ein offener Punkt:** der einzige Fernzugriffsweg des Projekts hängt an der `.env`, und die ist nicht versioniert. Geht sie verloren, startet das Dashboard auf `127.0.0.1`, ist über Tailscale nicht mehr erreichbar — und meldet dabei nichts. Bewusst nicht behoben, siehe `system/README_DIENSTE.md` (Abschnitt „Es lauscht nicht auf `localhost`").
...
| 4:50 *(seit TB-64, 21.09.2026; **eingetragen 21.09.2026, 10:50**, erster Lauf 22.09.; Zielmenge seit TB-76 aus `BACKLOG.md` gelesen)* | `system/nachtragswaechter.py` | ein Nachtrag unter `docs/projektfuehrung/nachtraege/` länger als einen Tag offen liegt, eine nach `_eingearbeitet/` verschobene Datei Nummern hat, die nicht in Backlog oder Journal stehen, oder eine Nummer doppelt vergeben ist (`system/README_NACHTRAGSWAECHTER.md`) |
...
### 4.8 Die gemeinsame Abrufschicht — die Entscheidungskerze (TB-38)

Am **15.09.2026** wurde der Datenbezug aller neun `forward_test.py` umgestellt. Es ist der erste Eingriff dieses Projekts in den Live-Pfad, ausdrücklich freigegeben und ausdrücklich **auf den Datenbezug beschränkt**: keine Signallogik, keine Schwelle, keine Positionsgrösse, kein Stop, `live_params.py` unberührt.

**Der Anlass** ist Nebenbefund N1 aus TB-35: die Bots entschieden auf `df.iloc[-1]`, also auf der zum Laufzeitpunkt **laufenden** Kerze. Bei `t3_supertrend` war das eine wenige Minuten alte Teilkerze einer Vier-Stunden-Strategie. **Der Backtest rechnet auf abgeschlossenen Kerzen** — beide rechneten also nicht dasselbe, und damit war die Vorbedingung D5 für den Echtgeld-Betrieb („Backtest-Pfad stimmt mit Papierpfad überein") gebrochen. Der Live-Record ist die einzige Out-of-Sample-Evidenz dieses Projekts; ein Holdout nach anderen Regeln als das Validierte ist als Bestätigung wenig wert.

**Die Regel** steht jetzt in **einer** Funktion, die den Filter für Backtest-Lesart und Papierpfad gleich beantwortet:

> **Entscheidungskerze ist die letzte Kerze, deren `close_time` vor der Startzeit des Laufs liegt.** Eine Kerze, deren `close_time` **genau** auf der Startzeit liegt, wird nicht genommen (striktes `<`).

`shared/entscheidungskerze.py` erfindet die Regel nicht neu: für Krypto ruft es `abrufschutz.abgeschlossen_maske` (TB-35) auf, die ihrerseits zeichengleich mit `kursdaten_neuaufbau.abgeschlossene_kerzen` (TB-34) ist. Neu ist **die Tür**, nicht die Regel: woher die Daten kommen, welche Startzeit gilt, und dass eine Abweichung gemeldet wird.

**Vier Entscheidungen, jede mit einer Fehlerrichtung dahinter:**

| | Entscheidung | Warum |
|---|---|---|
| **Ort** | eigenes Modul in `shared/`, das `abrufschutz` benutzt | `abrufschutz` schützt den **Schreibpfad** (zu langes Warten kostet nichts); hier geht es um den **Lesepfad** (zu langes Warten kostet einen Handelstag). Zwei entgegengesetzte Risikoprofile gehören nicht in eine Datei — die Regel selbst bleibt trotzdem genau einmal vorhanden |
| **Aktien** | echter NYSE-Kalender statt der `D 23:59:59`-Schranke | Die vier Aktien-Bots laufen **15 Minuten nach Handelsschluss** (22:15 Ortszeit gegen 20:00/21:00 UTC). Mit der konservativen Schranke aus `abrufschutz` wäre die Tageskerze dann noch „nicht fertig" und **jedes Signal käme einen Handelstag zu spät**. Die Frage ist hier zweiseitig teuer — genau der Fall, für den `notifications/boersenkalender.py` gebaut ist. Kann er nicht antworten, gilt die sichere Schranke **und der Lauf meldet es**; geraten wird nicht (das verbietet der Kopf von `boersenkalender.py` ausdrücklich) |
| **Startzeit** | einmal je Prozess eingefroren, als Argument überschreibbar, **keine Umgebungsvariable** | Ein Wert, der sich während des Laufs ändert, gäbe den ersten Symbolen Kerze N und den restlichen N+1 — und weil alle neun Bots ihr Positionslimit laufend hochzählen, hinge das Ergebnis dann von der Symbolreihenfolge ab. Eine vergessene `export`-Zeile wiederum würde neun Bots lautlos auf einer alten Kerze einfrieren, und ein eingefrorener Lauf sieht in den Zahlen aus wie „kein Signal" |
| **Meldung** | `notify.send_alert()` — **dieselbe** Leitung wie TB-32 — mit **denselben** Dämpfungsfunktionen aus `waechter_melden.py` | Eine Meldung in `logs/` ist keine Meldung. Ohne Dämpfung wäre es bei dauerhaft veraltetem `data/` eine Nachricht je Bot und Lauf; nach einer Woche wäre das dasselbe wie keine Meldung. Genau **einmal je Lauf**, alle betroffenen Symbole in einer Nachricht. Der Meldeteil steht vollständig in einem `try` und bringt den Bot nie zum Scheitern |

**Die Quelle ist jetzt `data/` mit Rückfall** (Entscheidung des Betreibers, 15.09.2026): fehlt in `data/` die erwartete Entscheidungskerze, ruft der Bot **genau seinen bisherigen Live-Abruf** auf und meldet das. Verworfen wurden „nur melden" (dieselbe Lücke wie TB-34, nur mit Benachrichtigung) und „blockieren" (verstösst gegen „Der Live-Betrieb darf nie blockiert werden"). ⚠️ **Der Rückfall-Abruf unterliegt derselben Regel** — sonst hätte der Ausnahmefall ein anderes Verhalten als der Normalfall, und Ausnahmefälle sind genau die, die niemand prüft.

⚠️ **Befund: heute greift der Rückfall bei sechs von neun Bots.** `data/` wird von **keinem** Cronjob aktuell gehalten — für die `fetch_*.py` gibt es keinen Eintrag, weil die Bots bisher ohnehin live abriefen. Gemessen am 15.09.2026 (`python3 shared/entscheidungskerze.py`): **199 von 223** Kursdateien tragen die erwartete Kerze nicht — alle 25 `_1h`, alle 24 `_4h` und alle 150 Aktien-Tagesdateien. Die 24 Krypto-Tagesdateien sind frisch. Solange das so bleibt, ist der effektive Datenweg unverändert der Endpunkt; die Umstellung wirkt trotzdem, weil der **Filter** auf beiden Wegen greift. Siehe Abschnitt 9, Punkt 24.

**Der Umstellungstag** wird je Bot festgehalten — in `docs/DATENLUECKEN.md` als Zäsur-Eintrag und maschinenlesbar in `docs/umstellungstag_entscheidungskerze.json` (`shared/umstellungstag.py`). ⚠️ **Papierpfade vor diesem Tag gelten für den Vergleich Backtest gegen Live als nicht vergleichbar** — der Vergleich beginnt für diese Bots **neu**, und bis zum Stichtag der Neuselektion am **13.12.2026** ist das das gesamte verfügbare Fenster.

Einzelheiten: `shared/README_ENTSCHEIDUNGSKERZE.md`, Ergebnis: `docs/ERGEBNIS_TB-38_abrufschicht.md`, Testauftrag: `docs/TESTAUFTRAG_TB-38_abrufschicht.md`, Selbsttests: `python3 shared/test_entscheidungskerze.py` (130 Prüfungen, davon 3 wirklich gestartete Bots und 9 Mutationsproben) und `python3 shared/test_umstellungstag.py` (25 Prüfungen).
...
- **Zielgröße der Auswahl seit TB-22: `robustness_score_chronologisch`** — dieselbe Formel wie `calculate_robustness_score`, nur mit `max_drawdown_chronologisch_pct` im Nenner (in `param_search_agent.py` aus den vorhandenen Spalten gerechnet, `multi_symbol_optimise.py` bleibt unberührt). Grund: `research/drawdown_reihenfolge/` hat gemessen, dass das bisherige Blockmaß bei drei Bots **negativ** mit dem echten Kapital-Drawdown rangkorreliert (−0,31 / −0,20 / −0,33) und bei keinem Bot näher daran liegt als die chronologische Reihenfolge. Die Übergangsregel „beide Maße ausweisen“ gilt für **Ausgaben und Berichte**; Agent 2 ist die benannte Ausnahme, weil er keine Zahl ausgibt, sondern eine **Auswahl** trifft. Beide Zahlen stehen weiterhin in der Tabelle, die das Modell sieht, und der Prompt sagt ausdrücklich, welche gilt; die Auswahl selbst trifft Code (`max(...)`), nicht das Modell
- Der Sendeweg ist seit TB-22 ein Parameter (`run_agent_search(..., call_fn=)`) — der Selbsttest `shared/test_agent2_zielfunktion.py` reicht eine Attrappe herein und kommt ohne einen einzigen echten API-Aufruf aus
- `agent_optimise.py` und `quarterly_review.py` geben bei jedem Vorschlag **fest verdrahtet** aus, auf welchem Maß ausgewählt wurde (`param_search_agent.zielmass_zeile()`) — aus demselben Grund, aus dem der Unvalidiert-Pflichthinweis eine Konstante ist: ein Modell könnte die Angabe variieren oder vergessen, ein Konstanten-String nicht
...
| Elliott-Aktien-Bot läuft weiter — **als Diversifikator**, nicht als Markt-Schläger (12.09.2026) | Der offene Punkt „weiterlaufen, überarbeiten oder abschalten?" ist damit entschieden. Die Parameter sind nachvollziehbar gewählt (Rang 10 von 116), nur der Zweck ist ein anderer als 2026-09-02 angenommen: gemessen wird der **Beitrag zum Portfolio**, nicht die Einzelrendite. Abschalten hätte eine Diversifikationswirkung aufgegeben, die niemand bisher widerlegt hat; „weil er den Markt schlägt" war nie wahr — es stand nur so in der Datei (Abschnitt 3.3, Befund TB-17). Geprüft wird im Oktober 2026 (Zwischenstand) und im Januar 2027 (belastbar), mit einem schärferen Kriterium: gleiche Zeit im Markt gegen das 95. Perzentil von Zufalls-Timing |
...
| **Zuteilungskaskade statt Dateireihenfolge** (TB-26) | Bei knappem Platz entschied bisher die Zeilenreihenfolge des Trade-DataFrames, und die ist die Symbolreihenfolge der Konfigurationsdatei — unsichtbar willkürlich und über acht von neun Bots messbar (TB-23). An ihre Stelle tritt eine vierstufige Regel: strategieeigene stetige Signalstärke (falls vorhanden), Diversifikationsbeitrag (60-Tage-Korrelation mit dem Buch, niedriger zuerst), Liquidität (20-Tage-Median-Dollar-Volumen), **protokollierter Zufall**. Der Zufall auf der letzten Stufe ist Absicht: *ehrliche Beliebigkeit schlägt versteckte* — er ist sichtbar willkürlich, reproduzierbar und entscheidet gemessen 6 von 3 704 Zuteilungen. Nach dem Symbolnamen zu sortieren wäre ebenfalls reproduzierbar gewesen, hätte aber A-Titel dauerhaft bevorzugt. **Zwei Bots bekommen einen Primärschlüssel** (`rsi_at_entry` bei den beiden RSI-2-Bots); der `fib_score` der Elliott-Bots taugt nicht dafür (nur fünf Werte, 38,6 %/72,7 % bleiben gleichauf) |
| Die Kaskade steht **einmal** in `shared/`, die neun `equity_simulation.py` werden **nicht** zusammengelegt (TB-26) | Die Regel neunmal einzubauen hätte den Befund von TB-23 neunmal reproduzierbar gemacht — also steht sie in `shared/zuteilung.py`, und die Bots behalten davon nur das Bot-Eigene (`SIGNALSPALTE`). Die ganzen Dateien zusammenzulegen wäre weiter gegangen: ihre `collect_all_trades()` haben verschiedene Signaturen, ihre `__main__`-Blöcke unterscheiden sich sachlich (BTC-Regimefilter bei `volatility_breakout_crypto`, PR #57), und `shared/determinismus.py`, `shared/ergebniskurven.py`, `shared/portfolio_overview.py` sowie mehrere `research/`-Programme hängen an der heutigen Form (ein `equity_simulation.py` je Bot, je Unterprozess importiert, mit `inspect.signature`-Abfragen darauf). `elliott_wave` behält aus demselben Grund seine Signatur **ohne** `max_concurrent_positions` |
| Die **Messkette** steht einmal in `shared/messkette.py`, die neun Dateien werden weiterhin **nicht** zusammengelegt (TB-28) | TB-27 hat gemessen, dass `calculate_max_drawdown()` und die Renditeformel in allen neun `equity_simulation.py` **zeichengleich** sind — die Messkette war also schon einheitlich, stand aber neunmal da. Sie wandert deshalb nach `shared/`, und zwar **nur sie**: die Dateien bleiben an ihren Pfaden, weil zwölf Stellen im Repo an der Signatur von `simulate_portfolio` erkennen, welcher Bot kein Positionslimit hat, drei Werkzeuge ihre Bot-Liste über die Dateiexistenz bilden und `notifications/manual_close.py` `ALLOCATION_PCT` als **Literal** aus drei dieser Dateien liest — im **schreibenden** Schliess-Dialog des Dashboards. Die Zusicherung war, dass sich **keine Zahl** ändert; belegt über SHA-256-gleiche Kurven vor und nach der Änderung, nicht über den Quelltext |
| `rendite_pct()` rundet **nicht**, `calculate_max_drawdown()` **doch** (TB-28) | Die drei Verbraucher der Renditeformel runden heute verschieden: die neun `__main__`-Blöcke formatieren nur (`:.2f`), `ergebniskurven.py` und `determinismus_lauf.py` runden auf zwei Stellen. Rundete die gemeinsame Funktion selbst, müsste einer von ihnen sein Verhalten aufgeben. Beim Drawdown ist es umgekehrt: die Rundung ist dort seit je Teil der ausgewiesenen Zahl, an über dreissig Stellen im Repo. Wer den ungerundeten Wert braucht, nimmt `max_drawdown_ungerundet()` |
...
4. **Der Elliott-Aktien-Bot schlägt Buy-and-Hold nicht — die Frage „weiterlaufen, überarbeiten oder abschalten?" ist seit dem 12.09.2026 entschieden.** Auf kausal sauberer Grundlage liegt er über den Gesamtzeitraum in Rendite **und** Calmar hinter passivem Halten (Abschnitt 3.3), und keine der 252 geprüften Parameterkombinationen ändert das. **Entschieden: er läuft weiter als Diversifikator**, nicht weil er den Markt schlägt, und wird künftig an seinem Beitrag zum Portfolio gemessen statt an der Einzelrendite (Abschnitt 8).

    **Offen bleibt die Prüfung, nicht die Entscheidung.** Fällig ist sie im **Quartals-Review Oktober 2026** (Zwischenstand) und belastbar eher im **Januar 2027**, mit einem schärferen Kriterium als bisher: der Bot bei **gleicher Zeit im Markt** gegen das **95. Perzentil von Zufalls-Timing**. Dieses Kriterium ist **noch nicht gemessen**. Es ist nötig, weil die Diversifikator-Begründung nicht selbstverständlich trägt: 95,4 % Zeit im Markt und +0,46 Korrelation zu Aktien-Buy-and-Hold (`research/exposure_messung/BERICHT.md`, Einzelheiten in Abschnitt 3.3).
...
17. **Der Fernzugriff aufs Dashboard hängt an der nicht versionierten `.env`** (Befund TB-16, 12.09.2026). `DASHBOARD_HOST` steht **nur** dort — nicht in der `plist`, nicht im Repo. Geht die `.env` verloren (Systemwechsel, Neuinstallation, versehentliches Löschen), startet das Dashboard auf der Voreinstellung `127.0.0.1` und ist über Tailscale nicht mehr erreichbar; der Dienst läuft dabei einwandfrei und meldet nichts. Zwei mögliche Wege stehen offen — `DASHBOARD_HOST` zusätzlich in `EnvironmentVariables` der `plist` führen, oder die **Namen** der benötigten `.env`-Schlüssel versioniert festhalten. **Bewusst nicht entschieden**, weil beides den laufenden Dienst bzw. den Umgang mit Zugangsdaten berührt. Einzelheiten in `system/README_DIENSTE.md`.

18. **`results/elliott_wave_stocks/EXPERIMENT_FINDINGS.md` führt durchweg Look-Ahead-Zahlen — seit dem 13.09.2026 mit Warnhinweis** (Befund TB-17). Die Datei ist die Quelle des Take-Profit-Eintrags vom 2026-09-03; alle ihre Kennzahlen, auch das Paar „mit/ohne Kursziel" (+1500,5 % gegen +3084,1 %), stammen von der look-ahead-behafteten Grundlage. Gültig ist für die Live-Konfiguration **+330,18 % bei −22,70 %** (`research/elliott_wave_params/BERICHT.md`) bzw. **+352,72 % bei −22,44 %** in der vom Bot erzeugten Kurve (PR #86).

    **Entschieden (Nutzer, 13.09.2026): Warnhinweis in die Datei.** Sie trägt jetzt einen Kasten am Dateianfang plus eine Zeile je betroffenem Abschnitt. **Ihre Zahlen wurden bewusst NICHT umgerechnet** — `research/elliott_wave_lookahead/` benutzt sie als *veröffentlichte Baseline* und weist damit nach, dass die Look-Ahead-Reproduktion den damaligen Lauf wirklich trifft (`decisions.py` → `PUBLISHED_CELLS`, `verify_baseline.py` → `PUBLISHED`). Wer die Tabellen dort überschreibt, zerstört diesen Nachweis; der Warnhinweis sagt das ausdrücklich.

    Die **Entscheidung** `USE_TAKE_PROFIT = False` hält — sie ruht seit PR #28 auf einer Rangfolge (unter den 116 Kombinationen, die die Mindestfilter bestehen, arbeiten die vorderen zehn praktisch ausnahmslos ohne festes Kursziel), nicht auf diesen Zahlen. Ein sauber gerechnetes Gegenstück zum Paar „mit/ohne Kursziel" existiert **nicht**; der gemessene Hebel der Entscheidung bleibt unbekannt. Die Empfehlung „unbegrenztes Positionslimit" aus derselben Datei ist auf dieser Grundlage nicht belastbar und wurde nie übernommen (siehe Punkt 1).

    **Nebenbefund, beim Anbringen des Hinweises aufgefallen:** Die abgelegte Ergebniskurve des Bots trug bis PR #86 die **+1500,53 % bei −1,90 %** — das ist die Zahl der Variante **mit** Take-Profit, obwohl `USE_TAKE_PROFIT` am 2026-09-03 auf `False` gesetzt wurde (die 469 ausgeführten Trades der alten Kurve entsprechen genau jener Zelle). Die Kurve war also aus **zwei** unabhängigen Gründen veraltet: dem Look-Ahead **und** einem Parameterwechsel, nach dem sie nie neu erzeugt wurde. Das ist ein Argument dafür, `shared/ergebniskurven.py` nach jeder Parameterübernahme laufen zu lassen, nicht nur nach einer Methodik-Korrektur.

19. **Die neun abgelegten `results/*/equity_curve.csv` sind seit dem 13.09.2026 veraltet** (siehe Nachtrag TB-26 oben). `shared/ergebniskurven.py` meldet **`9x ABWEICHEND`**, Rückgabewert 1 — zu Recht: TB-26 hat geändert, wer bei knappem Kapital den Platz bekommt, die Kurven wurden zuletzt am 12.09. (`0d1693e`) erzeugt. Montags-Mail, Dashboard-Portfoliosicht und praktisch jede Untersuchung unter `research/` lesen diese Dateien, solange ein Bot unter `MIN_LIVE_CLOSED_TRADES = 10` liegt — und das tun derzeit alle neun. **Zu erneuern mit `python3 shared/ergebniskurven.py --erzeugen`, als eigener, ausdrücklich freigegebener Schritt**, denn dabei verschieben sich ausgewiesene Zahlen. TB-28 hat den Zustand bestätigt und bewusst nicht behoben (der Auftrag untersagte es); er ist dort als Basislauf dokumentiert.
...
24. **`data/` wird von keinem Cronjob aktuell gehalten — deshalb greift der Rückfall der Abrufschicht bei sechs von neun Bots** (Befund TB-38, gemessen 15.09.2026). Die Entscheidung des Betreibers lautet „die Forward-Tests lesen `data/` statt live abzurufen". Das setzt voraus, dass etwas `data/` frisch hält, und das tut derzeit nichts: für keinen der acht `fetch_*.py` gibt es einen Crontab-Eintrag — sie wurden nur für Backtests von Hand gestartet, weil die Bots ohnehin live abriefen. `python3 shared/entscheidungskerze.py` meldet **199 von 223** Kursdateien ohne die erwartete Entscheidungskerze (alle `_1h`, alle `_4h`, alle 150 Aktien-Tagesdateien; die 24 Krypto-Tagesdateien sind frisch).

    **Das ist kein Schaden**: die Bots laufen weiter, rufen live ab wie bisher, und der Filter greift auf beiden Wegen gleich. Es heisst nur, dass der beabsichtigte Datenweg noch nicht wirkt und stattdessen täglich eine (gedämpfte) Telegram-Meldung entsteht. **Zu entscheiden vom Betreiber:** Crontab-Einträge für die `fetch_*.py` je Zeitrahmen, zeitlich **vor** den zugehörigen Bot-Läufen. TB-38 durfte die Crontab nicht anfassen; ein Vorschlag steht im Testauftrag, eingetragen ist er nicht.

25. **Der Schreibpfad und der Lesepfad unterscheiden sich um genau eine Millisekunde** (Entwurfsfolge TB-38, festgehalten in `shared/test_entscheidungskerze.py`, Abschnitt 11.2). `abrufschutz.nur_abgeschlossene` (was `fetch_*.py` schreibt) behält eine Kerze bei `schluss <= Stand`, `entscheidungskerze` nimmt sie bei `schluss < Startzeit`. Das strikte `<` ist die Vorgabe der Aufgabe („eine Kerze, deren `close_time` genau auf der Startzeit liegt, wird nicht genommen"). Unterschiedliche Antworten geben beide deshalb an genau einem Zeitpunkt je Kerze — der Millisekunde des Kerzenschlusses, die kein Cron-Eintrag trifft. **Bewusst nicht angeglichen**: eine Änderung an `abrufschutz` wäre eine Änderung am Schreibpfad und damit am Datenstand-Hash der Vorregistrierung. Der Test hält den Unterschied fest, damit ihn niemand für eine Panne hält.

26. **Die Lesart von `close_time` ist eine Auslegung — sie sollte einmal bestätigt werden** (TB-38). Die Aufgabe sagt „die letzte Kerze, deren `close_time` **vor** der Startzeit liegt". Was `close_time` bedeutet, entscheidet, ob die Regel sinnvoll oder absurd ist: mit der Binance-Lesart (`open + Länge − 1 ms`, Feld 6 der Rohkerze) nimmt ein Lauf um 12:00:00 die soeben fertige 08:00-Kerze eines Vier-Stunden-Charts; mit der „natürlichen" Lesart (`open + Länge`) dürfte er das **nicht** und müsste auf die 04:00-Kerze zurückgreifen — acht Stunden alt. Gebaut ist die Binance-Lesart, weil sie die Lesart der Datenquelle dieses Projekts ist und weil nur sie die Gleichheit mit dem Backtest herstellt. **Nur benannt, damit es einmal bewusst bestätigt wird.**
