...
| **K3x** | ⭐ **Ein langer `&&`-Block verschluckt seine eigenen Prüfungen.** Am 20.09. blieb ein verketteter Befehl bei `git push` hängen; `git status` und die Erfolgsmeldung dahinter liefen nie, und der Betreiber hielt die Arbeit für gesichert, obwohl sie es nicht war. ⇒ **Der Push steht am Ende der Kette oder in einem eigenen Befehl** |
