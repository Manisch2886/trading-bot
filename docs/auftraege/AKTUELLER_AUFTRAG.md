...
|---|---|---|
| **TB-90** | `docs/auftraege/MAC_TB-90_wegA_und_kostenmodul.md` | ⭐⭐ **CODE — Weg (A) und das Kostenmodul.** Registergrundlage 38.1 und 38.5, Betreiberfreigabe 22.09. ⚠️ **Block A:** In `faltenplan.py`, Funktion `_plan`, bei der Rollenzuweisung bekommt die Bestaetigungsfalte ihren Namen aus der eigenen Spanne (`von`/`bis_ausschliesslich`). ⛔ `_jahresfalten`, `auswertung.py`, `beispieldaten.py`, `registerbericht.py` UNBERUEHRT. Probe: alle NEUN Bots `2026-01-01/2026-09-01`, sonst genau ein String je Bot veraendert. ⚠️ **Block B:** neues `shared/handelskosten.py` (nur zwei Konstanten), die neun `backtest_*.py` ersetzen ihre Zuweisungen durch einen Import — `t3_supertrend` braucht zusaetzlich den `sys.path`-Block. ⛔ `forward_test.py` und `messgroessen.py` bleiben. Mutationsprobe mit vollstaendiger Ruecknahme. **Block C:** Faltenmengen der Tabellen gegen den Plan messen, vor UND nach Block A — nur messen. ⛔ Kein Abbild, keine Sonde anpassen, kein Vollzug, kein Registertext. ⚠️ `faltenplan.py` steht in der deny-Liste: EIN Genehmigungsklick ist gewollt. Drei JSON-Hashes unveraendert; der Hash von `faltenplan.py` aendert sich planmaessig (37.3) |

*Gesetzt 23.09.2026 (zuvor TB-89, erledigt mit `563fb54`, 23.09.2026; davor TB-88 `ec54618`, TB-87 `afe6192`, TB-86 `ee4e65c`;
