"""TB-114 A (Kopie von TB-113 c2_zitate.py, Abschnitt 45; Kopie von TB-110 d2_zitate.py; Quelle "git:<commit>:<pfad>" per git show): jedes eingesetzte Zitat im Register gegen seine Quelle (diff).
...
Aufruf aus der Repo-Wurzel; TB114_PROBE=<pfad> prueft eine Kopie.
"""
import json
import os
import subprocess
...
zitate = json.load(open("docs/belege/TB-114/zitate.json", encoding="utf-8"))
BEREICH = {"Abschnitt 45": (next(i for i, r in enumerate(reg) if r.startswith("## 45. ")), len(reg))}
