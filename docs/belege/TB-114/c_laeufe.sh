#!/bin/bash
# (Kopie von TB-113 a6_laeufe.sh, Kennung ersetzt; gemessen nach Block B von TB-114.)
# TB-114 C (Bauart TB-109 g1_g3.sh): Benchmark im Modus (Repo und frischer Klon) und Trockenlauf der neun Bots im Modus,
# mit dem Haken aus TB-104, am Endstand nach den Commits von A-C. Aufruf aus der Repo-Wurzel: bash docs/belege/TB-114/c_laeufe.sh <scratch>
set -u
SP="$1"; R=$(pwd); G="$R/docs/belege/TB-114/g_lauf.sh"
unset TB_SELEKTIONSWURZEL TB_SELEKTIONSHASH TB_SELEKTIONSCOMMIT
echo "# TB-114 C Modus-Laeufe, Repo, HEAD $(git rev-parse --short HEAD), $(date '+%Y-%m-%d %H:%M:%S %z')"
...
git clone -q "$R" "$K"
cd "$K" || exit 1
echo "# TB-114 C Modus-Lauf im frischen Klon <scratch>/a6_klon, HEAD $(git rev-parse --short HEAD), $(date '+%Y-%m-%d %H:%M:%S %z')"
