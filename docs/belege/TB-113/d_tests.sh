#!/bin/bash
# (Kopie von TB-112 d_tests.sh, nur die Kennung ersetzt.)
# (Kopie von TB-109 g4_tests.sh, dazu shared/test_arbeitsbaum_laufbereich.py; tb40_test_ und tb44_l_ mitgezaehlt)
# (Kopie von TB-107 g4_tests.sh, dazu shared/test_nulltrades_modus.py; die Zaehlung am Ende je Praefix)
# TB-113 D - alle Tests am Endstand, trading-env (3.9.6), je Datei rc, Dauer, Schlusszeile.
...
echo "# TB-113 D Tests, HEAD $(git rev-parse --short HEAD), Arbeitsbaum ausserhalb docs/ [$(git status --porcelain -- . ':!docs' | tr '\n' ' ')], $(date '+%Y-%m-%d %H:%M:%S %z')"
for t in research/faltenplan_neun/test_faltenplan_neun.py research/faltenplan_neun/test_erste_falte_trockenlauf.py \
         ...
         shared/test_main_gegenprobe.py shared/test_arbeitsbaum_laufbereich.py research/vorregistrierung/test_ersatzwerte.py \
