#!/bin/bash
# TB-107 G2 (Kopie von TB-106 g2_ausgaben.sh) - dieselben 8 Ausgaben wie TB-104 b0_ausgaben.sh OHNE Modus, vorher (Repo am Code-Stand f22f91e, vor jeder Aenderung) und nachher
...
#   bash docs/belege/TB-107/g2_ausgaben.sh <ziel>
set -u
Z="$1"; PY=/Users/jaquelineloffler/trading-bot/trading-env/bin/python3
if [ -e "$Z" ]; then echo "ABBRUCH: $Z existiert"; exit 1; fi
mkdir -p "$Z"
echo "# TB-107 G2 Ausgaben ohne Modus, HEAD $(git rev-parse --short HEAD), $(date '+%Y-%m-%d %H:%M:%S %z')"
