#!/bin/bash
# TB-107 G4 - alle Tests am Endstand, trading-env (3.9.6), je Datei rc, Dauer, Schlusszeile.
# Aufruf aus der Repo-Wurzel: bash docs/belege/TB-107/g4_tests.sh <scratch-logordner>
set -u
LOG="$1"; mkdir -p "$LOG"; PY=trading-env/bin/python3
unset TB_SELEKTIONSWURZEL TB_SELEKTIONSHASH TB_SELEKTIONSCOMMIT
echo "# TB-107 G4 Tests, HEAD $(git rev-parse --short HEAD), Arbeitsbaum ausserhalb docs/ [$(git status --porcelain -- . ':!docs' | tr '\n' ' ')], $(date '+%Y-%m-%d %H:%M:%S %z')"
for t in research/faltenplan_neun/test_faltenplan_neun.py research/faltenplan_neun/test_erste_falte_trockenlauf.py \
         ...
         research/universum_trockenlauf/test_zwischenablage.py shared/test_paths.py shared/test_strategy_paths.py \
...
echo "tb40_lauf_* in \$TMPDIR nach den Tests: $(ls -d "$TMPDIR"/tb40_lauf_* 2>/dev/null | wc -l | tr -d ' ')"
