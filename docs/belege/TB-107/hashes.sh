#!/bin/bash
# TB-107 - Hashes vorher/nachher (Vorlage TB-106 hashes.sh). Aufruf aus der Repo-Wurzel:
#   bash docs/belege/TB-107/hashes.sh <Bezeichnung>
...
# docs/belege/TB-107/5_alle_<Bezeichnung>.txt (Vergleich per diff), (6) Snapshot-Summenhash
...
echo "# TB-107 sha256 $1, HEAD $(git rev-parse --short HEAD), $(date '+%Y-%m-%d %H:%M:%S %z')"
echo "## (1) freigegeben"
...
echo "## (5) je versionierte Datei ausserhalb docs/ -> docs/belege/TB-107/5_alle_$1.txt"
git ls-files -- . ':!docs' | while read -r f; do shasum -a 256 "$f"; done > "docs/belege/TB-107/5_alle_$1.txt"
echo "   Anzahl: $(wc -l < docs/belege/TB-107/5_alle_$1.txt | tr -d ' ')"
