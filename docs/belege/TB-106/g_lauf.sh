#!/bin/bash
# TB-106 G1/G5 (Kopie von TB-105 f_lauf.sh) - Laeufe im Selektionsmodus auf den Snapshot der jeweiligen Wurzel, mit dem Haken
...
#   bash <wurzel>/docs/belege/TB-106/g_lauf.sh <laufordner> benchmark <name>   benchmark.py --ziel <laufordner>/<name>/benchmark.json
#   bash <wurzel>/docs/belege/TB-106/g_lauf.sh <laufordner> bot <name> <bot>   TB-103-Trockenlauf eines Bots
...
echo "# TB-106 $ART $NAME $BOT, Wurzel $PWD, HEAD $(git rev-parse HEAD), $(date '+%Y-%m-%d %H:%M:%S %z')"
