"""TB-112 A2 - Vereinigung der drei Laufbereichslisten (Ausgaben von docs/belege/TB-104/d1_listen.py) im
Format von docs/belege/TB-107/f1_laufbereich_vereinigung.txt, und der Unterschied zu den 81 Modulen von TB-107 F1.
...
with open(ziel, "w", encoding="utf-8") as f:
    f.write("# TB-112 A2 - Vereinigung der drei Lauf-Typen (Trockenlauf aller neun Bots, benchmark.py im Modus, "
            "auswertung.py nur Import), gemessen am Eingangs-Commit von TB-112 (e731207) am %s mit dem Werkzeug aus TB-104 D1\n"
...
alt = set()
for z in open("docs/belege/TB-107/f1_laufbereich_vereinigung.txt", encoding="utf-8"):
...
with open(ziel_u, "w", encoding="utf-8") as f:
    f.write("# TB-112 A2 Unterschied zur Laufbereichsmessung TB-107 F1 (f1_laufbereich_vereinigung.txt)\n")
    f.write("TB-107: %d Module, TB-112: %d Module, gemeinsam %d\n" % (len(alt), len(neu), len(alt & neu)))
    f.write("neu in TB-112 (%d):\n" % len(neu - alt))
    for p in sorted(neu - alt):
        f.write("  + %s\t%s\n" % (p, ",".join(typen[p])))
    f.write("nicht mehr in TB-112 (%d):\n" % len(alt - neu))
