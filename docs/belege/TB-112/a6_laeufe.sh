#!/bin/bash
# TB-112 A6 (Bauart TB-109 g1_g3.sh): Benchmark im Modus (Repo und frischer Klon) und Trockenlauf der neun Bots im Modus,
# mit dem Haken aus TB-104, am Endstand nach den Commits von A-C. Aufruf aus der Repo-Wurzel: bash docs/belege/TB-112/a6_laeufe.sh <scratch>
set -u
SP="$1"; R=$(pwd); G="$R/docs/belege/TB-112/g_lauf.sh"
unset TB_SELEKTIONSWURZEL TB_SELEKTIONSHASH TB_SELEKTIONSCOMMIT
echo "# TB-112 A6 Modus-Laeufe, Repo, HEAD $(git rev-parse --short HEAD), $(date '+%Y-%m-%d %H:%M:%S %z')"
...
git clone -q "$R" "$K"
cd "$K" || exit 1
echo "# TB-112 A6 Modus-Lauf im frischen Klon <scratch>/a6_klon, HEAD $(git rev-parse --short HEAD), $(date '+%Y-%m-%d %H:%M:%S %z')"
