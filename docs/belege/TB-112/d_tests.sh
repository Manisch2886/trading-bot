...
# TB-112 D - alle Tests am Endstand, trading-env (3.9.6), je Datei rc, Dauer, Schlusszeile.
...
echo "# TB-112 D Tests, HEAD $(git rev-parse --short HEAD), Arbeitsbaum ausserhalb docs/ [$(git status --porcelain -- . ':!docs' | tr '\n' ' ')], $(date '+%Y-%m-%d %H:%M:%S %z')"
