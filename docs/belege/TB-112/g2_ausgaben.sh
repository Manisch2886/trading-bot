#!/bin/bash
# TB-112 A5 (Kopie von TB-109 g2_ausgaben.sh, nur die Kennung ersetzt) - dieselben 8 Ausgaben wie TB-104 b0_ausgaben.sh OHNE Modus, vorher (Repo am Code-Stand f4d6d6d, vor jeder Aenderung) und nachher
...
#   bash docs/belege/TB-112/g2_ausgaben.sh <ziel>
set -u
Z="$1"; PY=/Users/jaquelineloffler/trading-bot/trading-env/bin/python3
if [ -e "$Z" ]; then echo "ABBRUCH: $Z existiert"; exit 1; fi
mkdir -p "$Z"
echo "# TB-112 A5 Ausgaben ohne Modus, HEAD $(git rev-parse --short HEAD), $(date '+%Y-%m-%d %H:%M:%S %z')"
