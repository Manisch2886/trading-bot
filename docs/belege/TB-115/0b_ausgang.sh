#!/bin/bash
# TB-115 0b - Ausgangswerte (Bauart TB-114 0b_ausgang.sh, ohne Trade-Listen-Zeilenzahlen):
# HEAD, register(), Sonde gegen das TB-114-Abbild 5e5ad109. Rein lesend.
# Aufruf aus der Repo-Wurzel: bash docs/belege/TB-115/0b_ausgang.sh <scratch> <marke>
set -u
...
echo "# TB-115 0b ($M), HEAD $(git rev-parse HEAD), $(date '+%Y-%m-%d %H:%M:%S %z')"
