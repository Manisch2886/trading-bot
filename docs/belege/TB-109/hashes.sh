#!/bin/bash
# TB-109 - Hashes vorher/nachher (Vorlage TB-107 hashes.sh). Aufruf aus der Repo-Wurzel:
#   bash docs/belege/TB-109/hashes.sh <Bezeichnung>
...
# docs/belege/TB-109/5_alle_<Bezeichnung>.txt (Vergleich per diff), (6) Snapshot-Summenhash
...
FREI="strategies/*/equity_simulation.py $U/universum_trockenlauf.py research/resolver_selektion/pfadvergleich.py shared/test_main_gegenprobe.py docs/belege/TB-104/d_auswerten.py $U/test_*.py shared/test_paths.py shared/test_strategy_paths.py shared/test_rueckfaelle_modus.py shared/test_startpruefungen.py"
echo "# TB-109 sha256 $1, HEAD $(git rev-parse --short HEAD), $(date '+%Y-%m-%d %H:%M:%S %z')"
echo "## (1) freigegeben"
...
for p in $V/faltenplan.py $V/benchmark.py $V/auswertung.py $V/herkunft.py $V/registerbericht.py $V/test_vorregistrierung.py $V/test_ersatzwerte.py $V/registerdaten.py $V/kennzahlen.py $V/messgroessen.py $V/pruefe_grenzsaetze.py \
         ...
         shared/strategy_paths.py $F/faltenplan_neun.py $F/faltenschranke_messung.py shared/regimewache.py shared/kurven_lauf.py shared/determinismus_lauf.py research/zuteilungskaskade/messung_primaerschluessel.py $U/loaderlauf.py $F/embargo_neun.py $F/erste_falte_trockenlauf.py shared/zuteilung.py shared/messkette.py \
         strategies/*/multi_symbol_optimise.py \
...
echo "## (5) je versionierte Datei ausserhalb docs/ -> docs/belege/TB-109/5_alle_$1.txt"
git ls-files -- . ':!docs' | while read -r f; do shasum -a 256 "$f"; done > "docs/belege/TB-109/5_alle_$1.txt"
echo "   Anzahl: $(wc -l < docs/belege/TB-109/5_alle_$1.txt | tr -d ' ')"
...
echo "## (10b) tb40_* in \$TMPDIR ($TMPDIR), je Praefix (nur gezaehlt)"
for pr in tb40_lauf_ tb40_faltenplan_ tb40_proben_ tb40_test_; do echo "   $pr $(ls -d "$TMPDIR"/${pr}* 2>/dev/null | wc -l | tr -d ' ')"; done
echo "   tb40_* gesamt $(ls -d "$TMPDIR"/tb40_* 2>/dev/null | wc -l | tr -d ' ')"
