#!/bin/bash
# TB-109 G1/G3 (Kopie von TB-107 g_lauf.sh) - Laeufe im Selektionsmodus auf den Snapshot der jeweiligen Wurzel, mit dem Haken
...
#   bash <wurzel>/docs/belege/TB-109/g_lauf.sh <laufordner> benchmark <name>   benchmark.py --ziel <laufordner>/<name>/benchmark.json
#   bash <wurzel>/docs/belege/TB-109/g_lauf.sh <laufordner> bot <name> <bot>   TB-103-Trockenlauf eines Bots
#   bash <wurzel>/docs/belege/TB-109/g_lauf.sh <laufordner> auswertung <name>  nur `import auswertung` (TB-104 D1 (c))
...
echo "# TB-109 $ART $NAME $BOT, Wurzel $PWD, HEAD $(git rev-parse HEAD), $(date '+%Y-%m-%d %H:%M:%S %z')"
