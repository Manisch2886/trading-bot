#!/bin/bash
# TB-105 F3 - dieselben 8 Ausgaben wie TB-104 b0_ausgaben.sh OHNE Modus, vorher (worktree b83e6b9) und nachher
...
mkdir -p "$Z"
echo "# TB-105 F3 Ausgaben ohne Modus, HEAD $(git rev-parse --short HEAD), $(date '+%Y-%m-%d %H:%M:%S %z')"
