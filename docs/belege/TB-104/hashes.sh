#!/bin/bash
# TB-104 - Hashes vorher/nachher (Vorlage TB-103 hashes.sh). Aufruf aus der Repo-Wurzel:
#   bash docs/belege/TB-104/hashes.sh <Bezeichnung>
# (1) freigegebene Dateien (registerbericht.py nur, falls per Auswahlkarte freigegeben; sonst Kontrolle), (2) gesperrte / nicht freigegebene Dateien einzeln,
...
FREI="research/vorregistrierung/benchmark.py research/vorregistrierung/faltenplan.py research/faltenplan_neun/faltenplan_neun.py research/faltenplan_neun/erste_falte_trockenlauf.py research/universum_trockenlauf/universum_trockenlauf.py research/universum_trockenlauf/loaderlauf.py research/faltenplan_neun/test_erste_falte_trockenlauf.py research/faltenplan_neun/test_faltenplan_neun.py research/faltenplan_neun/test_horizontbeginn.py research/universum_trockenlauf/test_universum_trockenlauf.py research/vorregistrierung/test_vorregistrierung.py research/vorregistrierung/registerbericht.py"
echo "# TB-104 sha256 $1, HEAD $(git rev-parse --short HEAD), $(date '+%Y-%m-%d %H:%M:%S %z')"
echo "## (1) freigegeben"
...
for p in shared/symbols_config.py strategies/*/stocks_symbols_config.py shared/ladeprotokoll.py \
         ...
         shared/paths.py shared/zuteilung.py research/vorregistrierung/herkunft.py \
         research/vorregistrierung/auswertung.py research/vorregistrierung/messgroessen.py \
         research/faltenplan_neun/embargo_neun.py shared/sperrlistensonde.py \
...
for p in research/vorregistrierung/ergebnisse/messgroessen.json \
         ...
         research/vorregistrierung/ergebnisse/benchmark_drawdowns_2026-09-23_nach_wegA.json \
