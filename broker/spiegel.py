...
def bot_db_pfad(bot: str) -> str:
    """Pfad der Bot-Datenbank - die Namensregel steht in broker/bot_db.py,
    damit beide Bruecken dieselbe benutzen."""
    return bot_db.pfad(bot, BASE_DIR)
...
def lies_trades(bot: str) -> list:
    """Alle Trades des Bots, schreibgeschuetzt gelesen - ueber broker/bot_db.py,
    die einzige Stelle im Projekt, die eine Bot-Datenbank zum Spiegeln oeffnet
    (mit mode=ro, siehe dortige Begruendung)."""
    try:
